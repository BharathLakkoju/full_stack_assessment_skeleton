...
> The solution involves normalizing the initial unstructured dataset into two related tables: `user` and `home`. Here's a brief explanation:
...
- **Fields**:
  ...
  - `email`: A unique, non-null field to store the user's email address.

#### 2. **Home Table**:

- **Purpose**: To store information related to homes.
- **Fields**:
  - `id`: A unique identifier for each home entry (primary key).
  - `username`: A foreign key referencing the `username` in the `user` table, establishing a relationship between users and their homes.
  - `street_address`, `state`, `zip`, `sqft`, `beds`, `baths`, `list_price`: Attributes related to each home.

#### 3. **Normalization Process**:

- **Data Split**: The original data is split into two tables. User-related information (`username` and `email`) goes into the `user` table, while home-related details are stored in the `home` table.
- **Foreign Key Relationship**: The `home` table includes a `username` field that acts as a foreign key, linking each home to a user in the `user` table. This ensures that each home is associated with a valid user, reducing redundancy and maintaining data integrity.

#### 4. **SQL Script**:

- **Creation**: The SQL script `99_final_db_dump.sql` is written to create these tables and insert the data.
- **Execution**: Running the script will transform the initial, unnormalized data into a well-structured relational database.

This approach optimizes the database design by reducing redundancy, ensuring data integrity, and clearly defining relationships between users and their homes.
